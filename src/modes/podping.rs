...
fn ws_url_with_start_block(ws_url: &str, start_block: u64) -> Result<String, String> {
    let mut url = reqwest::Url::parse(ws_url)
        .map_err(|e| format!("invalid PODPING_WS_URL {ws_url:?}: {e}"))?;
    url.query_pairs_mut()
        .append_pair("block", &start_block.to_string());
    Ok(url.to_string())
}
...
pub async fn run(
    ...
    loop {
        let replay_url = match ws_url_with_start_block(&ws_url, start_block) {
            Ok(url) => url,
            Err(e) => {
                eprintln!("podping: invalid replay URL: {e}");
                std::process::exit(1);
            }
        };
        eprintln!("podping: connecting to {replay_url}");

        let ws_result = tokio_tungstenite::connect_async(&replay_url).await;
            ...
            for entry in &block.p {
                ...
                if !should_accept(&entry.p) {
                    counters.entries_skipped_filter =
                        counters.entries_skipped_filter.saturating_add(1);
                    continue;
                }
...
mod tests {
    ...
    #[test]
    fn ws_url_with_start_block_appends_query() {
        let url = ws_url_with_start_block("wss://api.livewire.io/ws/podping", 123).expect("url");
        assert_eq!(url, "wss://api.livewire.io/ws/podping?block=123");
    }
}
